...
def do_oper(first_arg: float,second_arg: float,operation: str):
    '''do some operation to first and second arguments'''
    match operation:
        case '+': return first_arg + second_arg
        case '-': return first_arg - second_arg
        case '*': return first_arg * second_arg
        case '/': return first_arg / second_arg
        case '^': return first_arg ** second_arg
...
def to_calc(calc_example_str):
    ...
    for i in calc_example_arr:
        if i == '(':
            op_stack.append('(')
        elif i.isdigit() or i.replace('.','',1).isdigit():
            num_stack.append(i)
        elif i in '+-*/^':
            if len(op_stack) == 0 or op_stack[-1] == '(' or prior[i] > prior[op_stack[-1]]:
                ...
                while len(op_stack) > 0 and op_stack[-1] != '(' and prior[i] <= prior[op_stack[-1]]:
                    second_arg = float(num_stack.pop())
                    first_arg = float(num_stack.pop())
